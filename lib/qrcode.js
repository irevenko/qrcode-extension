...
(function () {
  ...
  QRCodeModel.prototype = {
    ...
    setupPositionProbePattern(row, col) { for (let r = -1; r <= 7; r++) { if (row + r <= -1 || this.moduleCount <= row + r) continue; for (let c = -1; c <= 7; c++) { if (col + c <= -1 || this.moduleCount <= col + c) continue; if ((r >= 0 && r <= 6 && (c == 0 || c == 6)) || (c >= 0 && c <= 6 && (r == 0 || r == 6)) || (r >= 2 && r <= 4 && c >= 2 && c <= 4)) { this.modules[row + r][col + c] = true; } else { this.modules[row + r][col + c] = false; } } } },
    getBestMaskPattern() {
  ...
  const svgDrawer = (function () {
    ...
    Drawing.prototype.draw = function (oQRCode) {
      ...
      const svg = makeSVG('svg', {
        viewBox: `0 0 ${String(nCount)} ${String(nCount)}`, width: '100%', height: '100%', fill: _htOption.colorLight,
      });
